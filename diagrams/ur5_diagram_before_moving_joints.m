...
[UNIT_SIZE, CYL_HALF_LENGTH, CYL_RADIUS] = diagrams.robot_plot(kin, q_0, auto_scale=true,...
...
diagrams.dot(zv)
diagrams.text(zv, "$\mathcal O_0 = \mathcal O_1$", align=">", margin=20);
diagrams.text(p_0x(:,2), "$\mathcal O_2$", align="v", margin=15);
diagrams.text(p_0x(:,3), "$\mathcal O_3$", align="v", margin=15);
diagrams.text(p_0x(:,4), "$\mathcal O_4$", align="v", margin=15);
diagrams.text(p_0x(:,5), "$\mathcal O_5$", align=">", margin=16);
diagrams.text(p_0x(:,6), "$\mathcal O_6$", align="^", margin=16);
diagrams.text(p_0T, "$\mathcal O_T $", align=">v");

diagrams.text(p_0x(:,1) - (UNIT_SIZE+CYL_HALF_LENGTH)*kin.H(:,1), "$h_1$", align=">v");
diagrams.text(p_0x(:,2) + (UNIT_SIZE+CYL_HALF_LENGTH)*kin.H(:,2), "$h_2$");
diagrams.text(p_0x(:,3) + (UNIT_SIZE+CYL_HALF_LENGTH)*kin.H(:,3), "$h_3$");
diagrams.text(p_0x(:,4) - (UNIT_SIZE+CYL_HALF_LENGTH)*kin.H(:,4), "$h_4$", align="^");
...
diagrams.arrow(p_0x(:,1)-(UNIT_SIZE+CYL_HALF_LENGTH)*kin.H(:,1), p_0x(:,1)-CYL_HALF_LENGTH*kin.H(:,1));
...
diagrams.arrow(p_0x(:,4)-(UNIT_SIZE+CYL_HALF_LENGTH)*kin.H(:,4), p_0x(:,4)-CYL_HALF_LENGTH*kin.H(:,4));
diagrams.arrow(p_0x(:,5)-(UNIT_SIZE+CYL_HALF_LENGTH)*kin.H(:,5), p_0x(:,5)-CYL_HALF_LENGTH*kin.H(:,5));
